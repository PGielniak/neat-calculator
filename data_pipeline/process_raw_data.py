...
def process_raw_sensor_data(raw_data_file_dir: str, labels_csv_path: str) -> pd.DataFrame:
    ...
    sliding_windows, feature_array, labels_array = create_sliding_windows(merged_data50Hz, window_size=128, step_size=64, sensor_cols=sensor_cols)
    del merged_data50Hz  # free up memory
    
    extracted_features = extract_features_from_windows(sliding_windows, feature_array, labels_array)
...
def create_sliding_windows(data_df: pd.DataFrame, window_size: int, step_size: int, sensor_cols: list[str]) -> Tuple[pd.DataFrame, np.ndarray, np.ndarray]:
    ...
    data_df["label"].ffill(inplace=True)
    logger.info("Number of sessions:", data_df["session_id"].nunique())
...
def extract_features_from_windows(data_df: pd.DataFrame, X_array: np.ndarray, y_array: np.ndarray) -> pd.DataFrame:
